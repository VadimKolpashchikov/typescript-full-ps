...
function generateWords(number: number, ...args: string[][]): string {
  let remainder: number = 0;
  let word: string = '';
  const words: string[] = args[1] ?? [];
  ...
  if (number < 20) {
    remainder = 0;
    word = LESS_THAN_TWENTY[number] as string;
  } else if (number < ONE_HUNDRED) {
    remainder = number % TEN;
    word = TENTHS_LESS_THAN_HUNDRED[Math.floor(number / TEN)] as string;
